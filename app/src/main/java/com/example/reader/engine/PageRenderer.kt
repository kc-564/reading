 ...
 * [Bitmap]. The pager then flips by blitting those baked bitmaps (GPU-composited), so
 * turning a page is a pure image swap with zero text re-measurement.
 *
 * [paginateChapter] and [renderPageBitmap] deliberately share the SAME paint + width +
 * line-spacing + alignment, and both call `setIncludePad` with the same value, so a
 * page's baked bitmap is pixel-identical to the character range that [paginateChapter]
